...
const ProductMsg = () => {
  ...
  useEffect(() => {
    if (!username) return;
    axios.get(`${API_BASE_URL}/get-userid/${username}`)
  ...
  useEffect(() => {
    if (!productId) return;
    setLoading(true);
    axios.get(`${API_BASE_URL}/product-buyers/${productId}`)
  ...
  useEffect(() => {
    ...
    setChatLoading(true);
    axios.get(`${API_BASE_URL}/chat/history/${productId}/${sellerId}/${selectedBuyer.user_id}`)
