...
const Sellitems = () => {
  ...
  // Fetch userId (sellerId)
  useEffect(() => {
    if (!username) return;
    const fetchUserId = async () => {
      try {
        const res = await axios.get(`http://${import.meta.env.vite_api_url}:8081/get-userid/${username}`);
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        setLoading(true);
        const res = await axios.get(`http://${import.meta.env.vite_api_url}:8081/user-products/${username}`);
  ...
  useEffect(() => {
    ...
    const fetchUnseen = async () => {
      try {
        const res = await axios.get(`http://${import.meta.env.vite_api_url}:8081/unseen-msg-count/${sellerId}`);
  ...
  // Fetch server time for drift correction
  useEffect(() => {
  ...
  // Auto-refresh active bids
  useEffect(() => {
    if (!userProducts || userProducts.length === 0) return;
    const interval = setInterval(() => {
      const current = Date.now();
      const active = userProducts.filter(
        (p) => Number(p.listing_type) === 1 && p.bid_end_time && new Date(p.bid_end_time).getTime() > current
      );
      active.forEach((p) => fetchRecentBids(p.id));
    }, 12000);
  ...
  const scheduleStopBidIn10 = async (productId) => {
    try {
      const res = await axios.post(`http://${import.meta.env.vite_api_url}:8081/bid/${productId}/stop-in-10`, { username });
      if (res.data?.success) {
        ...
        try {
          const response = await axios.get(`http://${import.meta.env.vite_api_url}:8081/user-products/${username}`);
  ...
  const handleDelete = async (productId) => {
    if (window.confirm("Are you sure you want to delete this product?")) {
      try {
        await axios.delete(`http://${import.meta.env.vite_api_url}:8081/delete-item/${productId}`);
                    ...
                    {Number(product.listing_type) === 1 && (
                      <div className="mt-1 text-sm">
                        <p className="text-yellow-700">
                          Highest bid:{" "}
                          {product.highest_bid ? `₹${Number(product.highest_bid).toFixed(2)}` : "—"}
                        </p>
                        <p className="text-yellow-700">
                          {product.bid_count
                          ...
                          <>
                            ...
                            <p
                              className={`text-xs font-medium ${
                                formatTimeLeft(product.bid_end_time) === "Ended"
                                  ? "text-red-600"
                                  : "text-green-600"
                              }`}
                            >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                    ...
                    <div className="bg-white rounded-lg shadow p-4">
                      ...
                      {product.bid_end_time && (
                        <div className="mt-2">
                          <p className="text-xs text-gray-500">
                            Ends at: {formatInTZ(product.bid_end_time)}
                          </p>
                          <p
                            className={`text-xs font-medium ${
                              formatTimeLeft(product.bid_end_time) === "Ended"
